...
[sat_kep,~] = uplanet(date2mjd2000(E_S.departure.earliest), i_flyby);
T_Sat = 2*pi*sqrt(sat_kep(1)^3/mu_Sun)/3600/24;
...
for i = 1:m
    for j = 1:n
        [E_S.deltaV(i, j), E_S.VF{i, j}, E_S.deltaVI(i, j), E_S.T_par(i, j)] =...
...
for i = 1:length(dates.flyby)
    PCPlot.flyby2(i) = datetime(mjd20002date(dates.flyby(i)));
end


pcp_dep = figure('Units','pixels', 'OuterPosition',[0 0 700 700]);
hold on;
contour(datenum(PCPlot.departure), datenum(PCPlot.flyby2), E_S.deltaV',...
    15:5:40, 'LineWidth',2);
datetick('x','dd/mm/yyyy','keeplimits','keepticks')
datetick('y','dd/mm/yyyy','keeplimits','keepticks')
xlabel('Departure date');
...
i = 1;
for k = 1:size(E_S.deltaV, 1)
    for j = 1:size(E_S.deltaV, 2)
 
        temp.time_fb = datetime(mjd20002date(dates.flyby(j)));
        temp.time_dep = datetime(mjd20002date(dates.departure(k)));

        if E_S.deltaV(k, j) < 35 && E_S.T_par(k, j) < ( seconds(diff([temp.time_dep; temp.time_fb] ) ) )
            E_S.data{i} = [dates.departure(k); dates.flyby(j); E_S.deltaV(k,j);...
...
lb = [min(guess(1)-365, date2mjd2000(E_S.departure.earliest)); guess(2)-365; guess(3)-365]; 
ub = [guess(1)+365; guess(2)+365; min(guess(3)+365, date2mjd2000(S_N.arrival.latest))]; 
...
plot.T_NEO = linspace(dates.arr, dates.arr+(3.5*365), 100);
